...
public class S2CMessageScreenUpdate extends WDPayload  {
	
	public static final CustomPacketPayload.Type<S2CMessageScreenUpdate> TYPE = 
		new CustomPacketPayload.Type<>(new ResourceLocation("webdisplays", "screen_update"));
    ...
    public void handle(IPayloadContext context) {
        if (!isClient(context)) return;

        context.enqueueWork(() -> {
            Level level = (Level) WebDisplays.PROXY.getWorld(context);
            BlockEntity be = level.getBlockEntity(pos);
            if (be instanceof ScreenBlockEntity tes) {
                control.handleClient(pos, side, tes, context);
            }
        });
