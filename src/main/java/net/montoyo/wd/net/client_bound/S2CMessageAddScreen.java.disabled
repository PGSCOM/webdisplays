...
public class S2CMessageAddScreen extends WDPayload {
	
	public static final CustomPacketPayload.Type<S2CMessageAddScreen> TYPE = 
		new CustomPacketPayload.Type<>(new ResourceLocation("webdisplays", "add_screen"));
	...
	public void handle(IPayloadContext context) {
		if (isClient(context)) {
			context.enqueueWork(() -> {
				Level lvl = (Level) WebDisplays.PROXY.getWorld(context);
